...
#启动浏览器
driver_path = os.path.dirname(os.getcwd()) + '\chromedriver.exe'
print(driver_path)
driver = webdriver.Chrome(driver_path)
driver.maximize_window()

def login():
    driver.get('http://wwww.jd.com')
    driver.find_element_by_class_name('link-login').click()
    driver.find_element_by_link_text('帐户登陆').click()
    driver.find_element_by_id('loginname').send_keys('qi414')
    driver.find_element_by_id('nloginpwd').send_keys('zhu7622511')
...
def to_goods_page(driver):
    driver.get('http://www.jd.com')
    computer_element = driver.find_elements_by_link_text('电脑')
    ActionChains(driver).move_to_element(computer_element).perform()
    time.sleep(2)
    driver.find_elements_by_link_text('笔记本').click()
...
def get_info_element_dict(info_element):
    ...
    computer_info_value = info_element.find_element_by_xpath('dl//dd[not(contains(@class),\'Ptable-tips\')]')
